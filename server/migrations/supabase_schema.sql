...
DROP POLICY IF EXISTS "Allow signalling select notification logs" ON public.notification_logs;
DROP POLICY IF EXISTS "Allow signalling insert notification logs" ON public.notification_logs;

CREATE POLICY "Allow signalling select notification logs" ON public.notification_logs FOR SELECT USING (public.is_signalling_server());
